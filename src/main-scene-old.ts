...
import { resetPresents } from './level';
...
export class MainSceneOld extends Phaser.Scene {
  playerName!: string;
  world!: IWorld;
  playerSystem!: System;
  ...
  create(): void {
    ...
    const startPositionHero: Position = {
      ...
      y: this.scale.height - adjustForPixelRatio(55) / 2 - adjustForPixelRatio(32),
    };
    ...
    this.startGameFn = () => {
      ...
      resetPresents(this.map, platformLayer, this.presentsGroup);
    ...
    console.log('this.map.widthInPixels', this.cameras.main.getBounds().width);
    ...
    this.physics.pause();
    createCountdown(this, 1, '#0653c7', this.startGameFn);
